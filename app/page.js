...
export default function Index() {
  ...
  return (
      <main className={styles.main} >
        ...
        <div onMouseMove={(e) => manageMouseMove(e)} className={styles.imageCursorSpace}  > {/* add id="cursor" for logo glitch alone with commented out code below*/}
          {
            [...Array(33).keys()].map((_, index) => {
              const ref = useRef(null);
              refs.push(ref);
              return <img id="cursor" key={index} onClick={() => console.log(refs)} ref={ref} src={`/images/${index}.jpg`}/>;
            })
          }
          <div className={styles.textBox} >
            {/*<div className={styles.textBox} id="cursor-inner">*/}
            <div >
          ...
          <div className={styles.navPanel} >
            ...
            <Link ref={textRef} className={styles.navItem}
                  href={'https://soundcloud.com/lewisgoing'}>soundcloud</Link>{"\n"}
            <Link ref={textRef} className={styles.navItem}
                  href={'https://open.spotify.com/artist/0Ll3UtLjep87qvzadZ2Cp4?si=kbOPgrmRQJ-nz1kS3dquQg'}>spotify</Link>{"\n"}
            <Link ref={textRef} className={styles.navItem}
                  href={'https://music.apple.com/us/artist/lewisgoing/1563419779'}>apple music </Link>{"\n"}
            {/*<FaApple size={20} />*/}
            <Link ref={textRef} className={styles.navItem}
                  href={'https://soundcloud.com/earth2lewis'}>livesets</Link>{"\n"}
            <Link ref={textRef} className={styles.navItem}
